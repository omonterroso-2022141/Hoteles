    ...
    const [formState, setFormState] = useState(() => {
        ...
        inputs.forEach(input => {
            initialState[input.field] = {
                isValid: input.field === 'passwordConfirm' 
                    ? validatePasswordConfirm(settings.password, settings.passwordConfirm) 
                    : eval(validate${input.field.charAt(0).toUpperCase() + input.field.slice(1)})(settings[input.field]),
                showError: false,
